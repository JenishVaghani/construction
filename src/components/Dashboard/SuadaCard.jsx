...
function SuadaCard({
  ...
  const show = SHOW
  return (
    <div className="w-64 border rounded-lg p-4 shadow-lg relative bg-white ml-4 mt-4">
      {/* Edit Icon */}
      <div className="absolute -top-5 right-2 bg-gray-300 p-2 rounded-full shadow-md cursor-pointer">
        <img src={Status === "Complete" ? show.img : edit.img} alt={Status === "Complete" ? show.name : edit.name} className="w-6 h-6" />
      </div>
